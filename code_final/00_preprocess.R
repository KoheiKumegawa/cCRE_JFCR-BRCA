...
sampleName <- paste0(pt_ls$ID, "_scatac.fragments.tsv.gz")
...
pre_arc <- filterDoublets(pre_arc) #total:33873 cells
...
for(i in c(1:nrow(pt_ls))){
  ...
  pre_arc$Method[idx] <- pt_ls$modality[i]
}
...
pdf("output/Plots/00_Dotplot_QC_cellnumbers.pdf", width = 6, height = 4)
p1
dev.off()
